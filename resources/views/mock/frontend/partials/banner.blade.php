<fieldset class="border border-solid border-gray-300 p-6 mt-6">
  <legend class="text-medium font-semibold px-2">Popular Categories</legend>
      ...
      <tbody>
        @foreach ($items as $item)
          <tr class="border-b border-gray-200 dark:border-gray-700" data-id="{{ $item->id }}">
            ...
            <td class="px-6 py-4">
              <x-select divClass="!mb-0" name="section_items[{!! $item->id !!}][category]" value="{!! $item->items['id'] !!}"
                default="Choose Category" options="{!! collect($categories) !!}" />
            </td>
        ...
        <tr class="hidden form-tr border-b border-gray-200 dark:border-gray-700">
          ...
          <td class="px-6 py-4">
            <x-select divClass="!mb-0" name="section_items[___][category]" disabled default="Choose Category"
